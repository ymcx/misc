...
pub async fn request(equation: &str) -> (String, bool) {
    ...
    if response.is_err() {
        return ("Couldn't connect to Ollama".to_string(), false);
    }
    ...
    if matches.is_none() {
        return ("Invalid response from Ollama".to_string(), false);
    }

    (matches.unwrap().get(1).unwrap().as_str().to_string(), true)
}
